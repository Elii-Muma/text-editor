...
TerminalView::TerminalView(sf::Font &font, int &charSize, sf::Vector2u &windowSize, unsigned int &height, bool isScreen) : m_isScreen{isScreen},
...
void TerminalView::init()
{
  ...
  m_cursorItem.setFillColor(themes::oceanBreeze.BACKGROUND);
  ...
  m_lineText.setFillColor(themes::oceanBreeze.BACKGROUND);
  m_lineText.setCharacterSize(m_characterSize);

  m_arrowText.setFont(m_font);
  m_arrowText.setFillColor(themes::oceanBreeze.BACKGROUND);
...
void TerminalView::drawLineText(sf::RenderWindow &window)
{
  if (!m_buffer.getInputBuffer().empty())
  {
    ...
    for (int i = 0; i < m_buffer.getInputBuffer().size(); i++)
    {
      ...
      m_lineText.setString(lineStr);

      // how do you prevent people from juzt typing that?? 
